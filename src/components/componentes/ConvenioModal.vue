    ...
    <div class="modal" :class="theme">
      ...
      <form @submit.prevent="handleSubmit" class="modal-form">
        <div class="form-group">
          ...
          <input v-model="form.nome" placeholder="Digite o nome do convênio" required />
        </div>
        
        <div class="form-actions">
          <button type="button" class="btn-cancel" @click="$emit('close')">Cancelar</button>
          <button type="submit" class="btn-submit">Criar Convênio</button>
        </div>
      </form>
    </div>
  </div>
...
const handleSubmit = async () => {
  try {
    ...
    window.alert("Convênio criado com sucesso!");
    emit("created");
    emit("close");
  } catch (err) {
    console.error("Erro ao criar convênio:", err);
    window.alert("Erro ao criar convênio");
  }
};
</script>

<style scoped>
/* Mesmo CSS do CriarServicoModal */
.modal-backdrop {
  position: fixed;
  inset: 0;
  display: flex;
